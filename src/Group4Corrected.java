...
public class Group4Corrected {
    ...
    public static String[] sort(String[] data) {
        ...
        for (String numberStr : data) {
            ...
            buckets.computeIfAbsent(sumPrimeFactors, k -> new ArrayList<>()).add(number);
        }
    
        // Step 3: Sort each bucket in descending order based on actual integer values
        ...
        for (Integer key : keysSorted) {
            for (Integer number : buckets.get(key)) {
                sortedData[index++] = String.valueOf(number);
            }
        }
